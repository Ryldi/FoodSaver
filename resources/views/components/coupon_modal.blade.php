    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form id="deleteProductForm" method="POST" action="{{ route('deletePromo') }}" class="p-4 md:p-5 text-center">
                ...
                <button data-modal-hide="delete-product-{{ $item->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium rounded-lg bg-accent hover:bg-white text-white hover:text-accent border hover:border-accent transition-all duration-500">No, cancel</button>
            </form>
        </div>
    </div>
</div>
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form action="{{ route('updatePromo', $item->id) }}" method="POST" class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-1">
                    <label for="disc_percentage" class="block mb-2 text-sm font-medium text-primary dark:text-white">Persentase Diskon</label>
                    <div class="flex">
                        <input type="number" name="disc_percentage" id="disc_percentage" aria-describedby="helper-text-explanation" value="{{ $item->percent }}" class="bg-gray-50 border border-gray-300 text-primary text-sm rounded-s-md focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="50" min="0" max="100" required />
                ...
                <div class="col-span-1">
                    ...
                    <input type="date" name="date" id="date" value="{{ $item->end->format('Y-m-d') }}" class="bg-gray-50 border border-gray-300 text-primary text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukkan Deskripsi" required="">
                </div>
                <div class="col-span-1">
                    <label for="max_disc" class="block mb-2 text-sm font-medium text-primary dark:text-white">Maksimal Diskon</label>
                    <div class="flex">
                        <span class="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-e-0 border-gray-300 rounded-s-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                            Rp
                        </span>
                        <input type="number" name="max_disc" id="max_disc" aria-describedby="helper-text-explanation" value="{{ $item->max_disc }}" class="bg-gray-50 border border-gray-300 text-primary text-sm rounded-e-md focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="20000" min="0" max="1000000" required />
                    </div>
                </div>
                <div class="col-span-1">
                    <label for="min_spend" class="block mb-2 text-sm font-medium text-primary dark:text-white">Minimal Pembelian</label>
                    <div class="flex">
                        <span class="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-e-0 border-gray-300 rounded-s-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                            Rp
                        </span>
                        <input type="number" name="min_spend" id="min_spend" aria-describedby="helper-text-explanation" value="{{ $item->min_spend }}" class="bg-gray-50 border border-gray-300 text-primary text-sm rounded-e-md focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="50000" min="0" max="1000000" required />
                    </div>
                </div>
                <div class="col-span-2">
                    <label for="description" class="block mb-2 text-sm font-medium text-primary dark:text-white">Deskripsi</label>
                    <input type="text" name="description" id="description" value="{{ $item->desc }}" class="block p-2.5 w-full text-sm text-primary bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Masukkan deskripsi promo"></input>                    
                </div>
                <input type="hidden" name="id" value="{{ $item->id }}">
                <button type="submit" class="col-span-2 border hover:border-accent py-1 px-6 rounded-lg hover:text-accent hover:bg-transparent text-white bg-accent transition-all duration-500 whitespace-nowrap">
                    Perbarui Promo
                </button>
            </form>
        </div>
