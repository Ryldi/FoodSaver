<div id="delete-product" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form id="deleteProductForm" method="POST" action="{{ route('deleteProduct', $product->id) }}" class="p-4 md:p-5 text-center">
                @csrf
                @method('DELETE')
                <svg class="mx-auto mb-4 text-gray-400 w-12 h-12 dark:text-gray-200" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 11V6m0 8h.01M19 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
                </svg>
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">@lang('deleteProductModal.confirmation_message')</h3>
                ...
                <button data-modal-hide="delete-product" type="submit" class="bg-red hover:bg-white text-white hover:text-red border hover:border-red transition-all duration-500 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                    @lang('deleteProductModal.yes')
                </button>
                <button data-modal-hide="delete-product" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium rounded-lg bg-accent hover:bg-white text-white hover:text-accent border hover:border-accent transition-all duration-500">
                    @lang('deleteProductModal.no')
                </button>
            </form>
        </div>
    </div>
